...
class Proximoslancamentos extends Component {
  constructor(props) {
    super(props)
  ...
  componentDidMount() {
    fetch(process.env.REACT_APP_API_URL_PLS)
      .then(res => res.json())
      .then((data) => {
  ...
  render() {
    return (
      <React.Fragment>
        <Header {...Proximoslancamentos} />
        <Main icon="rocket" title="Próximos Lançamentos" className="container">
